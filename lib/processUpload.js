 ...
 * `cardCount` is the total number of cards wanted for the whole file; `level` is the study level (see levels.js).
 * Resolves with { claimed, status } and never throws for per-file problems.
 */
async function processUpload(uploadId, { cardCount = DEFAULT_CARDS, level = DEFAULT_LEVEL } = {}) {
  ...
  try {
    ...
    // Split the requested card total across chunks, skip any that get zero, and run a few at a time.
    stage = "generate";
    const counts = allocateCards(chunks, total);
    const jobs = chunks.map((chunk, i) => ({ chunk, count: counts[i] })).filter((j) => j.count > 0);
    const perChunk = await mapPool(jobs, CONCURRENCY, (j) => generateForChunk(j.chunk, j.count, level));
